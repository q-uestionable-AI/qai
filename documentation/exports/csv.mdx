...
CSV exports produce a flat file with one row per retained audit finding, suitable for Excel,
Google Sheets, database import, and reporting workflows.

## Generating CSV

Generate CSV from the `ScanResult` returned by `run_scan()`:

```python
from ctpf.audit.reporting.csv_report import generate_csv_report

generate_csv_report(result, "results.csv")
```
...
<Note>
CSV flattens the `framework_ids` dict into one string column. For structured audit data, use the
JSON report generator or [NDJSON](/exports/ndjson). Controlled experiments write scenario evidence
bundles instead of finding CSV files.
</Note>

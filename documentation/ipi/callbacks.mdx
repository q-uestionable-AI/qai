...
5. The hit is persisted to `~/.qai/ipi.db` and logged to the console
...
All hits are stored in `~/.qai/ipi.db` (SQLite). Use `qai ipi export` to extract data as JSON for external analysis.

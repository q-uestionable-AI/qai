...
```bash Auto-select template from sweep findings
qai ipi generate http://localhost:8080 \
...
ENDPOINT_URL is the OpenAI-compatible API base URL (e.g., `http://localhost:8000/v1`). It can be provided as the first positional argument or entered interactively when running in a terminal. The service appends `/chat/completions` automatically.
...
<CodeGroup>
```bash Default sweep (all templates × obvious)
qai ipi sweep http://localhost:8000/v1 \
  --model Qwen/Qwen3.5-27B \
  --concurrency 4
```

```bash Targeted templates
qai ipi sweep http://localhost:8000/v1 \
  --model my-model \
...
```bash Obvious + citation comparison
qai ipi sweep http://localhost:8000/v1 \
  --model my-model \
...
```bash Dry run to inspect combinations
qai ipi sweep --dry-run --templates whois,email
```

```bash Export for import workflow
qai ipi sweep http://localhost:8000/v1 \
  --model my-model \

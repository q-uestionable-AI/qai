---
title: Overview
description: Import findings from Garak, PyRIT, SARIF-producing tools, scored-prompts data, and BIPIA benchmarks into the qai database
---
...
| `bipia` | [BIPIA](https://github.com/microsoft/BIPIA) | CSV benchmark data | Per-row IPI benchmark results with compliance scoring |
...
A CSV file from the [BIPIA benchmark](https://github.com/microsoft/BIPIA) (KDD 2025). Required columns: `category`, `prompt`, `response`, `complied`. Optional columns: `probe_id`, `description`, `score`, `model`.

Each row produces one finding with taxonomy bridging through the `ipi_probe` framework. Severity is derived from compliance rate using the same thresholds as the scored parser. When `score` is missing, it defaults to 1.0 if complied, 0.0 otherwise.

...
When the listener runs with `--tunnel cloudflare` (see [Remote callbacks via Cloudflare Tunnel](/ipi/callbacks#remote-callbacks-via-cloudflare-tunnel)), a few behaviors differ from a bare listener bound to `127.0.0.1` or a plain `--host 0.0.0.0`.

**Forwarded-header trust**. In tunnel mode the server resolves each hit's `source_ip` from the `CF-Connecting-IP` header rather than the TCP peer. This trust is scoped to tunnel mode only — a bare `--host 0.0.0.0` listener does **not** trust forwarded headers. The trust boundary is documented as a tunnel invariant in the architecture notes (commit `79be789`).

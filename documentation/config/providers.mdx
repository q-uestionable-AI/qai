...
CTPF Research Harness uses LLM providers for model-backed research workflows. Core proxy and local
state operations do not require a provider.
...
Store API keys in the OS keyring using `ctpf config set-credential`. The command prompts for the key with masked input — the key is never passed as a command-line argument.
...
```bash
ctpf config delete-credential anthropic
```
...
When a command needs a provider credential, the harness checks in this order:

1. **Environment variable** (e.g., `ANTHROPIC_API_KEY`)
2. **OS keyring** (stored via `ctpf config set-credential`)

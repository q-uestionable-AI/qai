...
description: Command reference for qai rxp
---

```bash
qai rxp [OPTIONS] COMMAND [ARGS]
```
...
```bash
qai rxp validate --profile hr-policy --verbose
```
...
```bash
qai rxp validate --profile hr-policy --model bge-small
```
...
```bash
qai rxp validate --profile hr-policy --model all --output results.json
```

...
qai uses SQLite with WAL mode for concurrent access and foreign key enforcement. The database lives at `~/.qai/qai.db` and auto-initializes with schema migrations on first access. Current schema version: **V10**.
...
See [External Tool Import](/imports/overview) for CLI usage and parser details.

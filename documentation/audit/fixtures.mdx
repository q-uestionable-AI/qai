...
The ctpf test suite includes intentionally vulnerable MCP server fixtures for scanner validation. These are internal test infrastructure — they live in the test suite, not as user-facing tools.
...
Scan it through the retained library:

```python
from ctpf.audit.orchestrator import run_scan
from ctpf.mcp.connection import MCPConnection

async with MCPConnection.stdio("python", ["my_test_server.py"]) as connection:
    result = await run_scan(connection)
```

<Note>
Each scanner targets one OWASP MCP Top 10 category. See [Scanner Catalog](/audit/scanners)
for registry names and subset selection with `check_names`.
</Note>

...
description: "How the frozen audit library maps findings to security taxonomies"
---

All 10 CTPF scanners map 1:1 to [OWASP MCP Top 10](https://owasp.org/www-project-mcp-top-10/) categories. `run_scan(connection)` runs all scanners by default; pass registry names through `check_names` to target a subset.
...
<Note>
The audit library reports static and active scanner observations. Controlled agent-response
experiments belong to the narrow `ctpf experiment` workflow and are not inferred from audit
categories alone.
</Note>
...
CTPF maps audit finding categories to [MITRE ATLAS](https://atlas.mitre.org/) technique IDs, connecting MCP-specific vulnerabilities to the broader adversarial ML threat landscape.
...
CTPF maps audit finding categories to [CWE](https://cwe.mitre.org/) weakness IDs for integration with vulnerability management workflows.
...
See [Scanner Coverage](/audit/scanners) for per-scanner detail and Python library usage.

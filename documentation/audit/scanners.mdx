...
q-ai ships 10 scanner modules, one per [OWASP MCP Top 10](https://owasp.org/www-project-mcp-top-10/) category. All scanners run by default on every `audit scan`. Use `--checks` to target specific modules.

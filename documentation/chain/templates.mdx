...
Escalate from RAG retrieval poisoning to agent control. Inject malicious documents into retrieval, trigger agent behavior deviation, then exploit trust boundaries between retrieval and generation.
...
category: rag_pipeline
description: >
  Escalate from RAG retrieval poisoning to agent control.
  Inject malicious documents, trigger behavior deviation,
  and exploit trust boundaries between retrieval and LLM.

steps:
  - id: poison-retrieval
    name: Poison RAG document store
    module: inject
    technique: retrieval_poisoning
    trust_boundary: document-to-agent
    on_success: trigger-deviation
    on_failure: abort

  - id: trigger-deviation
    name: Trigger agent behavior deviation
    module: cxp
    technique: behavior_deviation
    trust_boundary: retrieval-to-agent
    on_success: escalate-control
    on_failure: abort

  - id: escalate-control
    name: Escalate to full agent control
    module: inject
    technique: llm_instruction_override
    trust_boundary: llm-to-agent
    terminal: true
```

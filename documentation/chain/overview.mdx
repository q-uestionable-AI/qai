...
| Module | Purpose | Example Techniques |
|--------|---------|-------------------|
| `audit` | Vulnerability scanning | injection, auth, tool_poisoning |
| `inject` | Payload injection and escalation | output_injection, cross_tool_escalation, description_poisoning |
| `cxp` | Cross-execution patterns | agent behavior analysis |
| `ipi` | Prompt injection testing | direct_injection, indirect_injection |

## Trust Boundaries

Chains record which system boundaries are crossed during an attack:

- `tool_to_agent` — Tool output reaches agent reasoning
- `agent_to_tool` — Agent reasoning reaches tool invocation
- `agent_to_agent` — One agent influences another
- `llm_to_agent` — LLM output shapes agent decisions
- `user_to_system` — External input enters the system

...
|--------|----------|-------------|
| `--technique` | Yes | Technique ID (e.g., `backdoor`, `cmdexec`, `exfil`) |
...
| `--technique` | No* | Technique ID (required for file validation) |
| `--file` | No | Path to file(s) to validate (repeatable) |

\* Either `--result` or `--technique` is required.

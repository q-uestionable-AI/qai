...
Session files store complete MCP request/response sequences as JSON, providing a portable protocol
record for replay, debugging, and experiment evidence. A session alone is not a CTPF result or an
independently verified external effect.
...
```bash
ctpf proxy inspect --session-file session.json
```

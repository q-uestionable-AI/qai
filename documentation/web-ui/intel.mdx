...
Rows link to `/runs?run_id=<probe_run_id>`. As of Phase 6 this URL returns 200 directly (the old Intel redirect was removed).
...
Rows link to `/runs?run_id=<sweep_run_id>` and carry the `#sweep-run-<id>` anchor for deep-linking.
...
- `SelectedTemplate` — fresh. Latest sweep is under 7 days old, and one template clearly leads the pack. The button reads `Generate with <TEMPLATE>` and links to `/launcher` prefilled with the target and template.

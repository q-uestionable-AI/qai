...
- **Audit Categories**: Checkboxes for audit scanner categories. When the assess workflow runs, audit findings in selected categories inform which inject payloads are prioritized — templates whose `relevant_categories` match discovered findings run first. All techniques are selected by default.

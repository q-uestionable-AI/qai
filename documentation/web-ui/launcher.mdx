...
- **Inject Techniques**: Checkboxes for prompt injection methods (e.g., `parameter`, `argument`, `context`). All techniques are selected by default.
- **Advanced: Payload Library**: Toggle to manually select individual payloads, overriding technique filtering.
- **Audit Categories**: Checkboxes for scanner checks (e.g., `type_confusion`, `permission_confusion`). All categories are selected by default. Use "Select All" / "Clear All" for quick filtering.
...
- **Payload style**: How obvious the payload is (`obvious`, `subtle`, `invisible`).
- **Payload type**: Detection method (`callback` for outbound requests, `exfil` for embedded exfiltration).

...
<Note>
**Scope.** The Runs history list covers workflow runs — Assess an MCP Server, Test Document Ingestion, Test a Coding Assistant, and similar — producing audit, inject, IPI campaign, CXP, and chain results, plus import runs created by `qai import` or the Intel **Import Results** card. Probe runs (`module = ipi-probe`) and sweep runs (`module = ipi-sweep`) are **not** listed here; they belong to the Intel target workspace and surface on a target's detail page at `/intel/targets/<target_id>` under the `#probe-runs` and `#sweep-runs` sections. Direct `/runs?run_id=<id>` URLs for a probe or sweep run still return 200 post-Phase-6 (the old redirect is gone), but they render only the run's overview metadata — status, duration, target. The per-category probe breakdown and the template × style sweep matrix are not rendered under `/runs`; those live on the target detail page. Treat those URLs as deep links, not discovery surfaces.
</Note>
...
**Lifecycle**: the managed listener is **not** tied to the run's lifecycle. Workflow completion does **not** stop it — the listener (and its public tunnel) continues running until an operator clicks **Stop** on this panel. Tunnel exposure therefore persists past workflow completion; operators must explicitly tear it down when finished.

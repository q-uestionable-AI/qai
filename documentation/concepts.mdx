...
All audit findings map to four security frameworks:

### OWASP Agentic Top 10

The [OWASP Agentic Top 10](https://genai.owasp.org/resource/owasp-top-10-for-agentic-applications-for-2026/) addresses vulnerabilities in agentic AI systems. q-ai findings map to the Agentic Top 10 categories, providing guidance for securing broader AI agent deployments.

### MITRE ATLAS

[MITRE ATLAS](https://atlas.mitre.org/) (Adversarial Threat Landscape for AI Systems) catalogs tactics and techniques observed in real-world attacks on AI systems. q-ai maps audit findings to technique-level ATLAS IDs, providing a cross-reference between MCP-specific vulnerabilities and the broader adversarial ML threat landscape.

### CWE

q-ai maps findings to relevant CWE (Common Weakness Enumeration) IDs for vulnerability classification and tracking.

See [Framework Coverage](/audit/framework-coverage) for the full mapping tables across all four frameworks.
...
1. **Build** — Select insecure coding rules from the YAML rule catalog and assemble a context file from clean base templates + the selected rules
...
RXP validates the retrieval prerequisite for content injection — if a poisoned document doesn't appear in the top-k results, it never reaches the LLM context window.
...
| **rxp** | RAG retrieval layer | Whether adversarial documents appear in top-k retrieval results | Retrieval rate and poison rank scoring across embedding models |

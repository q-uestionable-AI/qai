...
Model strings follow the `provider/model` format used throughout qai. See [LLM Provider Configuration](/config/providers) for the full list of supported providers.
...
export QAI_ASSIST_MODEL="ollama/llama3.1"
```

```powershell Windows PowerShell
$env:QAI_ASSIST_PROVIDER = "ollama"
$env:QAI_ASSIST_MODEL = "ollama/llama3.1"
```
</CodeGroup>
...
<Note>
The embedding model runs locally via sentence-transformers regardless of which LLM provider you use. Changing it triggers a full reindex of the knowledge base on next query.
</Note>

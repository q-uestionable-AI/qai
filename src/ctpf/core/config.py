"""Configuration management for CTPF.

Supports a hybrid approach:
- OS keyring for credentials (API keys)
- YAML file (~/.ctpf/config.yaml) for non-secret settings
...
def load_config(config_path: Path | None = None) -> dict:
    ...
    Args:
        config_path: Path to config file. Defaults to ~/.ctpf/config.yaml.
...
def resolve(
    ...
    # Check DB settings
    from ctpf.core.db import get_connection, get_setting

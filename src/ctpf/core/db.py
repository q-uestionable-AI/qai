"""Database connection manager and CRUD helpers for CTPF."""
...
def get_connection(
    ...
    Args:
        db_path: Path to database file. Defaults to ~/.ctpf/ctpf.db.
...
def _collect_files_for_cleanup(
    ...
    Only returns paths that resolve to locations inside ~/.ctpf to prevent
...
def _validate_file_paths(candidates: list[str]) -> list[str]:
    ...
    Returns:
        Paths that resolve inside ~/.ctpf.
    """
    allowed_base = _CTPF_DATA_DIR.resolve()
    safe: list[str] = []

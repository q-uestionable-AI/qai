...
from ctpf.proxy.correlation import is_notification
...
from ctpf.proxy.tui.messages import (
    MessageForwarded,
    MessageHeld,
    MessageReceived,
    PipelineError,
...
from ctpf.proxy.tui.widgets.message_detail import MessageDetailPanel
from ctpf.proxy.tui.widgets.message_list import MessageListPanel, MessageSelected
from ctpf.proxy.tui.widgets.status_bar import ProxyStatusBar
...
class ProxyApp(App[None]):
    ...
    def __init__(
        ...
        if listen_transport:
            self.title = (
                f"ctpf-proxy \u2014 {listen_transport} \u2014 "
                f"listening on {LISTEN_HOST}:{listen_port} \u2192 {target}"
            )
        else:
            self.title = f"ctpf-proxy \u2014 {transport.value} \u2014 {target}"
    ...
    def action_replay_message(self) -> None:
        ...
        if not self.server_command and not self.server_url:
            self.notify("Replay requires a target (command or URL)", severity="warning")
            return
    ...
    async def _replay_worker(
        ...
        """
        try:
            from ctpf.proxy.adapters.sse import SseServerAdapter as _SseAdapter
            from ctpf.proxy.adapters.stdio import (
                StdioServerAdapter as _StdioAdapter,
            )
            from ctpf.proxy.adapters.streamable_http import (
                StreamableHttpServerAdapter as _HttpAdapter,
            )
            from ctpf.proxy.constants import stdio_subprocess_env
            ...
            if self.server_command:
                ...
                adapter_cm = _HttpAdapter(url=self.server_url)
            else:
                self.post_message(
                    ReplayCompleted(
                        result=ReplayResult(
                            ...
                            response=None,
                            error="No replay target configured",
                            duration_ms=0.0,
                        ),
            ...
            async with adapter_cm as server_adapter:

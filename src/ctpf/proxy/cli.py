...
if TYPE_CHECKING:
    from ctpf.proxy.replay import ReplayResult, ReplaySessionResult
...
def start(
    ...
    from ctpf.proxy.tui.app import ProxyApp
...
def replay(
    ...
    from ctpf.mcp.models import Direction
...
async def _run_replay(
    ...
    from ctpf.proxy.replay import ReplaySessionResult, replay_messages
...
async def _run_replay_http(
    ...
    """Run the replay against an SSE or Streamable HTTP server adapter.

    Args:
        url: The server endpoint URL.
        transport: Transport type string ("sse" or "streamable_http").
        messages: List of ProxyMessages to replay.
        timeout: Per-message response timeout in seconds.
        auto_handshake: Whether to send a synthetic handshake.
    ...
    Returns:
        A ReplaySessionResult with results for each replayed message.
    """
    from ctpf.proxy.adapters.sse import SseServerAdapter
    from ctpf.proxy.adapters.streamable_http import StreamableHttpServerAdapter
    from ctpf.proxy.replay import ReplaySessionResult, replay_messages

    adapter_cm: SseServerAdapter | StreamableHttpServerAdapter
    if transport == "sse":
        adapter_cm = SseServerAdapter(url=url)
    else:
        adapter_cm = StreamableHttpServerAdapter(url=url)

    async with adapter_cm as adapter:
        results = await replay_messages(
            messages, adapter, timeout=timeout, auto_handshake=auto_handshake
        )
    return ReplaySessionResult(results=results, target_url=url)

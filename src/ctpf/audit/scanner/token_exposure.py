"""Scanner for token mismanagement and secret exposure (token_exposure).
...
Checks:
    CTPF-TOK-001: Sensitive parameter names in tool schemas
    CTPF-TOK-002: Secret patterns in tool responses
    CTPF-TOK-003: Secrets in error responses
    CTPF-TOK-004: Environment variable leakage

References:
    - token_exposure category
"""
...
from ctpf.audit.scanner.base import (
    BaseScanner,
    ScanContext,
    ScanFinding,
    Severity,
...
logger = logging.getLogger("ctpf.audit.scanner.token_exposure")
...
class TokenExposureScanner(BaseScanner):
    """Scanner for token mismanagement and secret exposure.
    ...
    async def scan(self, context: ScanContext) -> list[ScanFinding]:
        ...
        Returns:
            List of ScanFindings for token exposure issues.
        """
    ...
    def _check_sensitive_params(self, tool: dict[str, Any]) -> list[ScanFinding]:
        ...
        Returns:
            List of ScanFindings for sensitive parameter names.
        """
        findings: list[ScanFinding] = []
    ...
    async def _check_secrets_in_response(
        ...
        Returns:
            List of ScanFindings for secrets in tool responses.
        """
        findings: list[ScanFinding] = []
        tool_name = tool.get("name", "unknown")
    ...
    async def _check_secrets_in_errors(
        ...
        Returns:
            List of ScanFindings for secrets in error responses.
        """
        findings: list[ScanFinding] = []
        tool_name = tool.get("name", "unknown")
    ...
    def _build_env_var_findings(
        ...
        Returns:
            List of ScanFindings for env var leakage.
        """
        findings: list[ScanFinding] = []

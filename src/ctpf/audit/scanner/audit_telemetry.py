...
class AuditTelemetryScanner(BaseScanner):
    ...
    def _check_logging_capability(
        ...
        has_logging_capability = capabilities.get("logging", False)
        ...
        if not has_logging_capability and not has_log_resources:
            return [
                ScanFinding(
                    ...
                    severity=Severity.MEDIUM,
                    evidence=(
                        "Server capabilities show logging disabled, no log-related resources found"
                    ),
                    remediation=(

...
from ctpf.audit.scanner.base import BaseScanner, ScanContext, ScanFinding, Severity

logger = logging.getLogger("ctpf.audit.scanner.tool_poisoning")
...
def _check_homoglyphs(name: str) -> list[dict[str, str]]:
    ...
    homoglyph_map: dict[str, str] = {
        ...
        "\u0441": "c",  # Cyrillic small es (U+0441)
        "\u0445": "x",  # Cyrillic small ha (U+0445)
        "\u0443": "y",  # Cyrillic small u (U+0443)
        "\u03b1": "a",  # Greek small alpha (U+03B1)
        "\u03bf": "o",  # Greek small omicron (U+03BF)
        "\u03c1": "p",  # Greek small rho (U+03C1)
        "\u0391": "A",  # Greek capital alpha (U+0391)
        "\u0392": "B",  # Greek capital beta (U+0392)
        "\u0395": "E",  # Greek capital epsilon (U+0395)
        "\u0397": "H",  # Greek capital eta (U+0397)
        "\u039a": "K",  # Greek capital kappa (U+039A)
        "\u039c": "M",  # Greek capital mu (U+039C)
        "\u039d": "N",  # Greek capital nu (U+039D)
        "\u039f": "O",  # Greek capital omicron (U+039F)
        "\u03a1": "P",  # Greek capital rho (U+03A1)
        "\u03a4": "T",  # Greek capital tau (U+03A4)
        "\u03a7": "X",  # Greek capital chi (U+03A7)
    }
...
class ToolPoisoningScanner(BaseScanner):
    ...
    def _check_embedded_instructions(self, tool: dict[str, Any]) -> list[ScanFinding]:
    ...
    def _check_hidden_unicode(self, tool: dict[str, Any]) -> list[ScanFinding]:
    ...
    def _check_homoglyphs(self, tool: dict[str, Any]) -> list[ScanFinding]:
        """Check tool name for homoglyph characters (visual spoofing).

        Attackers can register tools with names like 'read_file' (Cyrillic i)
    ...
    def _check_description_length(self, tool: dict[str, Any]) -> ScanFinding | None:
    ...
    def _check_duplicate_tools(self, tools: list[dict[str, Any]]) -> list[ScanFinding]:

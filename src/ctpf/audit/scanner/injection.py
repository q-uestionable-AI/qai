"""Command Injection scanner (command_injection).
...
Maps to: command_injection category
"""
...
from ctpf.mcp.payloads.injection import InjectionPayload, get_injection_payloads

logger = logging.getLogger("ctpf.audit.scanner.injection")
...
class InjectionScanner(BaseScanner):
    """Scanner for command injection via MCP tools.
    ...
    async def scan(self, context: ScanContext) -> list[ScanFinding]:
        ...
        Returns:
            List of ScanFindings for each confirmed injection.
        ...
        for tool in context.tools:
            ...
            logger.info(
                "Testing %s -- %d string params x %d payloads",
                tool_name,
                len(string_params),
                len(payloads),
            )
    ...
    async def _test_param(
        ...
        Returns:
            A ScanFinding if injection is confirmed, None otherwise.
        """
    ...
    def _detect_canary(
        ...
        Returns:
            A ScanFinding if canary found (and not just reflected), None otherwise.
        """
        if payload.canary and payload.canary in response_text:
            ...
            return ScanFinding(
                rule_id=f"CTPF-INJ-{cwe}-{payload.technique}",
    ...
    def _detect_pattern(
        ...
        Returns:
            A ScanFinding if a pattern matched, None otherwise.
        """
    ...
    def _detect_error_based(
        ...
        Returns:
            A ScanFinding (MEDIUM severity) if error references the payload,
            None otherwise.
        """

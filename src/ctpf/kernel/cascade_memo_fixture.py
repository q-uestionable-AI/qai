...
- ``CTPF_CASCADE_RUN_ID`` — **required by default**; scopes memo and sink files.
- ``CTPF_CASCADE_REQUIRE_RUN_ID`` — default ``1``; set ``0`` for unscoped smoke.
- ``CTPF_CASCADE_RESET=1`` — delete active memo + sink on process start.
...
_WORK_DIRNAME = "ctpf-cascade-memo"
...
def _run_id() -> str:
    ...
    return os.environ.get("CTPF_CASCADE_RUN_ID", "").strip()
...
def _require_run_id_enabled() -> bool:
    ...
    raw = os.environ.get("CTPF_CASCADE_REQUIRE_RUN_ID", "1").strip().lower()
...
def _enforce_run_id_requirement() -> None:
    ...
    if _run_id():
        return
    sys.stderr.write(
        "CTPF_CASCADE_RUN_ID is required for cascade trials "
        "(set CTPF_CASCADE_REQUIRE_RUN_ID=0 to allow unscoped paths).\n"
    )
    raise SystemExit(2)


def _maybe_reset_on_start() -> None:
    """Clear active memo and sink when ``CTPF_CASCADE_RESET`` is truthy."""
    flag = os.environ.get("CTPF_CASCADE_RESET", "").strip().lower()
    if flag in _TRUEISH:

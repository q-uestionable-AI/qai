...
CTPF reconnect Phase 1 deleted ``src/ctpf/server`` and its former tests.
...
def test_legacy_server_package_removed() -> None:
    """Assert the legacy ``ctpf.server`` package is no longer importable."""
    assert importlib.util.find_spec("ctpf.server") is None
